...
/**
 * 解析test_开头的预驼峰字段名，提取目标子串（保留驼峰格式）
 * @param a_szSrc 原始字段名（如test_nType_type）
 * @param a_szDst 输出解析后的子串（如nType）
 * @param a_iDstSize 输出缓冲区大小
 * @return TDR_SUCCESS/失败码
 */
static int tdr_parse_test_prefix_name(IN const char* a_szSrc, OUT char* a_szDst, IN int a_iDstSize)
{
	if (NULL == a_szSrc || NULL == a_szDst || a_iDstSize <= 0)
	{
		return TDR_ERRIMPLE_INVALID_PARAM;
	}

	// 1. 定义特殊前缀
	const char* pszTestPrefix = "test_";
	int iPrefixLen = strlen(pszTestPrefix);

	// 2. 判断是否以test_开头
	if (strncmp(a_szSrc, pszTestPrefix, iPrefixLen) != 0)
	{
		return TDR_ERRIMPLE_NOT_MATCH; // 非test_开头，返回不匹配
	}

	// 3. 截取test_后的部分（如test_nType_type → nType_type）
	const char* pszRest = a_szSrc + iPrefixLen;
	if ('\0' == pszRest[0])
	{
		return TDR_ERRIMPLE_INVALID_PARAM; // test_后无内容，非法
	}

	// 4. 提取目标子串：按需求定义规则（核心！可根据实际场景调整）
	// 【规则示例1】取test_后第一个_前的部分（test_nType_type → nType）
	char szTemp[TDR_NAME_LEN] = { 0 };
	strncpy(szTemp, pszRest, sizeof(szTemp) - 1);
	char* pszSep = strchr(szTemp, '_'); // 找到第一个_的位置
	if (pszSep != NULL)
	{
		*pszSep = '\0'; // 截断到第一个_，得到nType（保留原有驼峰）
	}

	// 【规则示例2】直接提取指定关键词（如强制取nType，适用于固定格式）
	// const char* pszTarget = "nType";
	// if (strstr(pszRest, pszTarget) != NULL)
	// {
	//     strncpy(szTemp, pszTarget, sizeof(szTemp) - 1);
	// }

	// 【规则示例3】取test_后所有内容（test_nType → nType）
	// strncpy(szTemp, pszRest, sizeof(szTemp) - 1);

	// 5. 将提取的子串复制到输出（保留驼峰）
	strncpy(a_szDst, szTemp, a_iDstSize - 1);
	a_szDst[a_iDstSize - 1] = '\0'; // 确保字符串结束
	...
	IN LPTDRMETALIB a_pstLib, IN LPTDRHPPRULE a_pstRule)
{
	...
	// ======================================
	// 新增：处理test_开头的特殊字段名（预驼峰）
	// ======================================
	char szSpecialName[TDR_NAME_LEN] = { 0 };
	iRet = tdr_parse_test_prefix_name(a_pstEntry->szName, szSpecialName, sizeof(szSpecialName));
	if (TDR_SUCCESS == iRet)
	{
		// 解析成功：直接使用提取的驼峰名（如nType）
		TDR_STRNCPY(szName, szSpecialName, sizeof(szName));

		// 强制清空所有前缀（类型、指针、自定义），完全脱离原有规则
		pszTypePrefix = "";
		pszPointPrefix = "";
		pszCustomPrefix = "";

		// 强制跳过大小写调整（因为已经是驼峰，无需修改）
		// 注：这里直接跳过原有大小写逻辑，所以szName保持提取的驼峰格式
	}
	else
	{
		// 非test_开头：保留原有全部逻辑
