...
void tdr_md5hash_buffer(OUT unsigned char szoutHashBuffer[TDR_MD5_DIGEST_LENGTH], const unsigned char *pszInBuffer, unsigned int uLength)
{
	...
	*/
	...
	MD5Init( md5Info );
	MD5Update( md5Info, pszInBuffer, uLength );
	MD5Final( szoutHashBuffer, md5Info );
...
char *tdr_md5hash2str(IN unsigned char szoutHashBuffer[TDR_MD5_DIGEST_LENGTH], OUT char *pszStr, IN int iStrSize)
{
	...
	assert(NULL != pszStr);
	assert(iStrSize >= TDR_MD5_DIGEST_LENGTH*2+1);
	
	pchChar = pszStr;
	for (i = 0;  i < TDR_MD5_DIGEST_LENGTH; i++)
	{
		ch = ((szoutHashBuffer[i] & 0xF0) >> 4);
		if (ch <= 0x09)
		{
			*pchChar++ = '0' + ch;
		}else
...
int tdr_str2md5hash(OUT unsigned char szoutHashBuffer[TDR_MD5_DIGEST_LENGTH], IN const char *pszStr)
{
	...
	assert(NULL != pszStr);
	assert(TDR_MD5_DIGEST_LENGTH*2 <= strlen(pszStr));
	
	if (TDR_MD5_DIGEST_LENGTH*2 > strlen(pszStr))
	{
		return -1;
	}
	...
	for (i = 0; i < TDR_MD5_DIGEST_LENGTH; i++)
	{
		ch = (char)tolower(*pchChar);
		if (('0' <= ch) && ('9' >= ch))
		{
			szoutHashBuffer[i] = ch - '0';
		}else if (('a' <= ch) && ('f' >= ch)) 
		{
			szoutHashBuffer[i] = ch - 'a' + 0x0a;
		}else
		{
			return -2;
		}
